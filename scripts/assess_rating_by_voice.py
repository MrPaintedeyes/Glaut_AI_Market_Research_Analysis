...
file_path = r"C:user_file_path\synthetic_dataset.csv"

# Load the dataset
df = pd.read_csv(file_path)

# Split the data based on the Voice variable
group_0 = df[df['VOICE'] == 0]['RATING']
group_1 = df[df['VOICE'] == 1]['RATING']
...
sns.boxplot(x='VOICE', y='RATING', data=df)
...
if positive_t_test is True:
  print("The difference in means for the variable "Rating" considering the two levels of the variable VOICE is statistically significant")
else:
  print("There is not a statistically significant difference between the means of the variable "Rating" considering the two levels of the variable VOICE")
