...
contingency_table = pd.crosstab(df['FOLLOWUP'], df['comparison_result'])
...
df_long = contingency_table.reset_index().melt(id_vars='FOLLOWUP', var_name='comparison_result', value_name='Count')
...
plt.subplot(1, 2, 1)
sns.barplot(data=df_long, x='FOLLOWUP', y='Count', hue='comparison_result', palette='viridis')
plt.title('Result of the Transcripts Quality Comparison within each FOLLOWUP level')
plt.xlabel('FOLLOWUP')
plt.ylabel('Count')
