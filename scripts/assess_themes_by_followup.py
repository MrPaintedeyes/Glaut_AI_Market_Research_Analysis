...
file_path = r"C:\Users\l440\Downloads\mydata.csv"

# Load the dataset
df = pd.read_csv(file_path)

# Convert 'VOICE' to string if it's not already
df['FOLLOWUP'] = df['FOLLOWUP'].astype(str)
...
unique_voices = df['FOLLOWUP'].unique()  # Ensure VOICE is treated as string
palette = {voice: color for voice, color in zip(unique_voices, ['skyblue', 'salmon'])}

sns.boxplot(x='FOLLOWUP', y='# themes / respondent', data=df, palette=palette)
plt.title('The Number of Themes per Survey Conditioned by the Presence of Followups')
plt.show()
